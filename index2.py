...
def update_output(start_date, end_date, val):
    ...
    for i in data:
        if datetime.strptime(i.get('opensAt')[:10], '%Y-%m-%d') >= datetime.strptime(start_date, '%Y-%m-%d') and datetime.strptime(i.get('opensAt')[:10], '%Y-%m-%d') <= datetime.strptime(end_date, '%Y-%m-%d') and i.get('company').get('sectorId') in val and i['compDet'][0]['gross']>=1600000 \
                and ('asso' in i['title'].lower() or 'anal' in i['title'].lower() or 'mana' in i['title'].lower() or 'con' in i['title'].lower() or 'prod' in i['title'].lower()) and ('data' not in i['title'].lower() or 'eng' not in i['title'].lower() or 'resear' not in i['title'].lower()) :
            lis.append([i.get('id'), i.get('opensAt'), i.get('closesAt'), i.get('title'), i.get('company').get('name')])
            try:
    ...
    for i in lis:
        bodylis=[]
        try:
            bodylis = [html.H4('Day: '+i[5])]
        except:
            pass
        res.append(dbc.Card([
            dbc.CardHeader(dcc.Link(html.H5(i[3]), href='/code/{}'.format(i[0]), target='_blank')),
            dbc.CardBody(bodylis+[
                html.H5('Opens at: ' + i[1][:10]+' Closes at: ' + i[2][:10]),

            ]),
            dbc.CardFooter(html.H5(i[4]))
